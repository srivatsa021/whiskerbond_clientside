...
const Services = () => {
  ...
  const getTierDescription = (tier: TierKey) =>
    TIER_CONFIG[tier].points.join(" • ");
  ...
  const submitForm = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (form.price < form.basePrice) {
        toast({
          title: "Invalid price",
          description: "Price must be at least the base price",
          variant: "destructive",
        });
        return;
      }
      ...
      if (form.tier === "custom") {
        if (!form.duration || !form.duration.trim()) {
          toast({
            ...
            description: "Please enter duration (e.g., 2 weeks)",
            variant: "destructive",
          });
          return;
        }
        ...
        if (
          ...
          (!form.daysPerWeek || form.daysPerWeek < 1 || form.daysPerWeek > 6)
        ) {
          toast({
            title: "Invalid days/week",
            description: "Days per week must be 1–6",
            variant: "destructive",
          });
          return;
        }
        ...
        description:
          form.tier !== "custom"
            ? form.description && form.description.trim().length > 0
              ? form.description
              : TIER_CONFIG[form.tier as TierKey].points.join(" • ")
            : form.description || "",
        tier: form.tier,
  ...
  const confirmDelete = async () => {
    ...
    try {
      await trainerServicesApi.deleteService(serviceToDelete.id);

      // Immediately update local state to remove the service
      setServices((prevServices) =>
        prevServices.filter((s) => s._id !== serviceToDelete.id)
      );
      ...
      <Card className={active ? "border-green-500 bg-green-50" : ""}>
        ...
        <CardContent className="space-y-3">
          ...
          <div className="text-sm font-semibold">
            Base price: ₹{cfg.basePrice}
          </div>
          {active && (
            <div className="text-sm">
              Your price: <span className="font-bold">₹{svc?.price}</span>
            </div>
          )}
          ...
          <form onSubmit={submitForm} className="space-y-3">
            ...
            {form.tier !== "custom" && (
              <div className="grid grid-cols-2 gap-2">
                <div>
                  ...
                  <Input
                    value={TIER_CONFIG[form.tier as TierKey].label}
                    readOnly
                  />
                </div>
                <div>
                ...
                <div className="col-span-2">
                  ...
                  <Input
                    value={(TIER_CONFIG as any)[form.tier as TierKey].plan}
                    readOnly
                  />
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 gap-2">
                ...
                <div>
                  ...
                  <Input
                    ...
                    placeholder="e.g., 2 weeks or 10 days"
                  />
                </div>
                <div>
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setForm({
                          ...form,
                          daysPerWeek: parseInt(e.target.value || "0", 10),
                        })
                      }
                    />
                  </div>
            ...
            <div>
              <Label htmlFor="price" className="text-sm">
                Your Price (₹){" "}
                {form.basePrice > 0 && (
                  <span className="text-gray-500">(≥ {form.basePrice})</span>
                )}
              </Label>
              <Input
                id="price"
                    ...
                    onChange={(e) => {
                      ...
                      setForm((prev) => ({
                        ...
                        serviceTypes: checked
                          ? Array.from(
                              new Set([...(prev.serviceTypes || []), "shop"])
                            )
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      setForm((prev) => ({
                        ...
                        serviceTypes: checked
                          ? Array.from(
                              new Set([...(prev.serviceTypes || []), "house"])
                            )
                          : (prev.serviceTypes || []).filter(
                              (t) => t !== "house"
                            ),
            ...
            {services.map((svc) => (
              <Card key={svc._id}>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    {svc.tier !== "custom" && (
                      <span className="text-xs px-2 py-1 rounded bg-gray-100">
                        {TIER_CONFIG[svc.tier as TierKey].label}
                      </span>
                    )}
                  </div>
                </CardHeader>
                <CardContent className="space-y-2">
                  ...
                  <div className="text-sm">
                    Types:{" "}
                    {svc.serviceTypes?.length
                      ? svc.serviceTypes.join(", ")
                      : "—"}
                  </div>
                  ...
                  <div className="flex gap-2 pt-2">
                          ...
                          description:
                            svc.tier !== "custom"
                              ? svc.description &&
                                ...
                                ? svc.description
                                : getTierDescription(svc.tier as TierKey)
                              : svc.description || "",
                          tier: svc.tier,
                    ...
                    <Button
                      ...
                      variant="outline"
                      onClick={() =>
                        handleDeleteClick(svc._id, svc.serviceName)
                      }
                    >
